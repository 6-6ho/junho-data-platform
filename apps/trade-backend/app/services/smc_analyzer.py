...
class SMCAnalyzer:
    ...
    def detect_structure(self, df: pd.DataFrame) -> Dict:
        ...
        # Analyze last few swings to determine structure
        if len(swings) >= 4:
            ...
            if len(highs) == 2 and len(lows) == 2:
                # Check for HH (Higher High) and HL (Higher Low) -> Bullish
                hh = highs[1]['price'] > highs[0]['price']
                hl = lows[1]['price'] > lows[0]['price']
                
                # Check for LH (Lower High) and LL (Lower Low) -> Bearish
                ...
                if hh and hl:
                    ...
                    trend = "neutral" 
                    # Could be expanding wedge or triangle
