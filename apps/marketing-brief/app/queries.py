...
def _ser(row) -> dict | None:
    if not row:
        return None
    d = dict(row)
    for k in ("trends", "producthunt", "hackernews", "naver"):
        v = d.get(k)
        if isinstance(v, str):
            d[k] = json.loads(v or "[]")
