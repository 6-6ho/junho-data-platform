...
|---------|----------|----------|-------|
| **PostgreSQL** | `postgres` | `postgres` | DB: `app` |
| **Grafana** | `admin` | `admin` | Initial setup |
| **Airflow** | `admin` | `admin` | Web UI |
| **MinIO** | `minio` | `minio123` | S3 Compatible |
...
- **해결**: `docker-compose.yml`에서 `spark-job-runner` 메모리 제한 상향 (2G -> 3G).
