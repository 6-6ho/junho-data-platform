...
| **Today's Shop Stats** | 금일 생성된 Shop 통계 데이터 수 | `COUNT(*)` FROM `shop_brand_stats_log`<br>WHERE `window_start >= CURRENT_DATE` | N/A (Volume) |
| **Today's Trade Signals** | 금일 감지된 급등(Pump) 시그널 수 | `COUNT(*)` FROM `movers_latest`<br>WHERE `event_time >= CURRENT_DATE`<br>AND `type='rise'` | N/A (Volatility) |
